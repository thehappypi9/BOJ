...
bool Tester(vector<int>*, int);
bool Test(vector<int>*, int, int, int);
...
// 입력
    ...
    int M; cin >> M;
    vector<int> DoesHeHave(M);
    for (int j = 0; j < M; j++) { cin >> DoesHeHave[j]; }
...
// 확인 및 출력
    for (int j = 0; j < M; j++) {
        cout << Tester(&HeHas, DoesHeHave[j]) << " ";
    }
