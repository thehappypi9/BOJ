...
int main() {
    ...
    sort(num.begin(), num.end());  // C++ 내장함수로 바로 정렬해주기.
    mean /= N;

// mean 반올림 후 출력
    if (mean-int(mean) != 0) {
        if (mean >= 0) {
            if (mean-int(mean) < 0.5) { mean = int(mean); }
            else { mean = int(mean) + 1;}
        }
        else {
            if (mean-(int(mean)-1) < 0.5) { mean = int(mean)-1; }
            else { mean = int(mean); }
        }
    }
    cout << mean << endl;
...
// 최빈값
    ...
    int already = 2;
    for (int i = 0; i < 8001; i++) {
        if (counting[i] == maxcounting && already > 0) {
            mode = i-4000;
            already--;
        }
    }
