// "시간초과"
// dfs_sudoku 함수에서 매번 새로운 스도쿠 판을 hard copy 해서 시간이 많이 걸린 듯.
...
void dfs_sudoku(vector<vector<int>>* sudoku, int tmprow, int tmpcol, int number, vector<vector<int>>* original, int donerow) {
...
// 복제한 스도쿠 업데이트
    sudokuforhere[tmprow][tmpcol] = number;
...
// 새로운 빈 칸을 찾아서 dfs.
    ...
    for (int i = donerow+1; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            if ( sudokuforhere[i][j] == 0 ) {
                newrow = i;
                newcol = j;
                for (int k = 1; k <= 9; k++) { // 1부터 9까지 중에서,
                    if (safe_checker(&sudokuforhere, newrow, newcol, k)) { // 넣어도 괜찮은 거면 넣어본다.
                        dfs_sudoku(&sudokuforhere, newrow, newcol, k, original, i-1);
                    }
                }
                i = 10;
                break;
...
// 더 이상 빈 칸이 없을 때.
    if (newrow == -1) {
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                (*original)[i][j] = sudokuforhere[i][j];
            }
        }
        return;
    }
...
// 가장 처음에 있는 빈칸에 넣어보기.
    for (int k = 1; k <= 9; k++) {
        if (safe_checker(sudoku, tmprow, tmpcol, k)) { // 1부터 9까지 중에서,
            dfs_sudoku(sudoku, tmprow, tmpcol, k, sudoku, -1); // 넣어도 괜찮은 거면 넣어본다.
        }
    }
