...
int recursion(const char *s, int l, int r, int* t){
    (*t)++;
    if(l >= r) return 1;
    else if(s[l] != s[r]) return 0;
...
int isPalindrome(const char *s, int* t){
    return recursion(s, 0, strlen(s)-1, t);
}
...
int main(){
    ...
    int T;
    cin >> T;
    char S[1010];    // 1000개까지인데 넉넉하게 1010으로 잡음. (1000으로 하니 틀렸다.)
    int time;
    for (int i = 0; i < T; i++) {
        time = 0;
        cin >> S;
        cout << isPalindrome(S, &time) << " ";
        cout << time << "\n";
    }
